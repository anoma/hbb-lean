...
/-- A chain of `n + 1` possible worlds whose anchor correlation rows are pairwise
distinct. The index `n` counts row *changes* along the chain, so a single world
is a chain of index `0`. -/
...
theorem depthChain_bounded {M : Model S P}
    ...
    (h : DepthChain M R a n) : n ≤ PreHistory.height M.history.val := by
  obtain ⟨ws, hpossible, hchain, _⟩ := h
  have lower : ∀ (k : Nat) (hk : k < n + 1),
...
theorem exists_maxDepth (M : Model S P)
  ...
  exact bounded_maximum (DepthChain M R a) (PreHistory.height M.history.val)
    (depthChain_zero M R a) (fun _ h => depthChain_bounded h)

/-- The actual maximum number of row changes along a causal chain of the model,
not a supplied proof budget. This is one less than the maximum chain length. -/
...
theorem maxDepth_bounded (M : Model S P)
    (R : World P S.EventType → S.Value → S.Value → Prop) (a : S.Value) :
    maxDepth M R a ≤ PreHistory.height M.history.val :=
