...
theorem capped_round_progress (h : BaseProtocol M σ) {l v : S.Value}
    (huod : ∀ p : P, UncontestedOrDelivered M σ ⟨p, †, M.history.val⟩ l v)
    (hzero : ⊨[M] □ᶠ↓[[l]] (σ.live ∧ᶠ σ.vote l v 0)) :
    ∀ n, n ≤ maxDepth M (Correlated M σ.correlationSymb) l →
      ⊨[M] □ᶠ↓[[l]] (σ.live ∧ᶠ σ.vote l v n) := by
  intro n
  induction n with
  ...
  | succ n ih =>
    ...
    apply ThyHBB1.boxPast_live_of_eventual_quorum h.thyLive
      (σ.voteCertificate l v n) (σ.vote l v (n + 1)) l
      (live_boxPast_nests (M := M) (hTheory := h.thyLive) (hAllowed := .event _) (hQuorum := hp))
    intro w hw
    apply Sat.imp_intro
    intro hboth
    obtain ⟨hlive, hcert⟩ := (Sat.and (M := M) (w := w) _ _).1 hboth
    exact h.voteSuccForward hw hn hlive (huod w.place) hcert

/-- Once rank zero is established, capped progress reaches delivery. -/
theorem deliver_of_live_zero_quorum (h : BaseProtocol M σ) {l v : S.Value}
    (huod : ∀ p : P, UncontestedOrDelivered M σ ⟨p, †, M.history.val⟩ l v)
    (hzero : ⊨[M] □ᶠ↓[[l]] (σ.live ∧ᶠ σ.vote l v 0)) :
    ⊨[M] σ.live ⇒ᶠ ↕ᶠ (σ.deliver l v) := by
  have hlast := capped_round_progress h huod hzero
    (maxDepth M (Correlated M σ.correlationSymb) l) (Nat.le_refl _)
  have hknow := live_eventually_knows_box (M := M) (hTheory := h.thyLive) (hAllowed := .event _) (hQuorum := hlast)
  have hforward : □W⊨[M]
      (σ.live ∧ᶠ σ.voteCertificate l v (maxDepth M (Correlated M σ.correlationSymb) l)) ⇒ᶠ
        ↕ᶠ (σ.deliver l v) := by
    intro w hw
    apply Sat.imp_intro
    intro hboth
    obtain ⟨hlive, hcert⟩ := (Sat.and (M := M) (w := w) _ _).1 hboth
    exact h.deliverForward hw hlive hcert
  intro p
  apply Sat.imp_intro
  intro hlive
  exact ThyHBB1.live_sometime_consequent_at h.thyLive hforward hlive
    (Sat.imp_elim (M := M) (w := ⟨p, †, M.history.val⟩) (φ := σ.live)
      (ψ := ↕ᶠ (σ.voteCertificate l v (maxDepth M (Correlated M σ.correlationSymb) l)))
      (hknow p) hlive)

/-- A unique proposal known at a live event initializes the round-zero quorum. -/
theorem live_zero_quorum_of_unique_proposal (h : BaseProtocol M σ) {l v : S.Value}
    ...
    ⊨[M] □ᶠ↓[[l]] (σ.live ∧ᶠ σ.vote l v 0) := by
  ...
  apply ThyHBB1.boxPast_live_of_eventual_quorum h.thyLive
    (σ.echoCertificate l v) (σ.vote l v 0) l
  ...
  intro hboth
  obtain ⟨hlive, hcert⟩ := (Sat.and (M := M) (w := w) _ _).1 hboth
  exact h.voteZeroEchoForward hw hlive (huod w.place) hcert

/-- Provenance makes every competing vote incompatible with the unique proposal. -/
theorem uncontestedOrDelivered_of_unique_proposal (h : BaseProtocol M σ) {l v : S.Value}
    (hUnique : UniqueOccurrence M σ.propose)
    (hKnown : Occurs M (σ.propose v)) :
    ∀ p, UncontestedOrDelivered M σ (finalWorld M p) l v := by
...
theorem livenessOne (h : BaseProtocol M σ) {l v : S.Value}
  ...
  have hKnown : Occurs M (σ.propose v) :=
      ⟨t, History.subset_of_happensBefore (H := M.history) ⟨e.place, e.event, he⟩ t ht, hprop⟩
  have huod := uncontestedOrDelivered_of_unique_proposal (l := l) h hUnique hKnown
  have hzero := live_zero_quorum_of_unique_proposal h huod
